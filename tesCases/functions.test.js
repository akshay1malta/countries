...
test("Returns all countries", () => {
  expect(getAllCountriesNames()).toEqual(
...
describe("Currency related tests", () => {
  it("Get Currency Data test", () => {
    const currenCy = "USD";
    expect(JSON.stringify(getCurrencybyCountryCode(currenCy))).toEqual(
      JSON.stringify(currencyData[currenCy.toUpperCase()])
    );
  });
  ...
  it("Negative Currency Test", () => {
    ...
    expect(JSON.stringify(getCurrencybyCountryCode(actualCurrency))).not.toBe(
      JSON.stringify(currencyData[falsycurrency.toUpperCase()])
    );
  });
});
...
test("Returns all states", () => {
  expect(getAllCountryStateNames()).toEqual(
...
test("Returns all country codes", () => {
  expect(getAllCountriesCallingCode()).toEqual(
...
test("Returns all countries flag", () => {
  expect(getAllCountriesFlag()).toEqual(
...
test("Returns all countries languages", () => {
  expect(getAllCountriesLanguages()).toEqual(
    countriesLanguageData.map((country) => {
      return country;
    })
  );
});
